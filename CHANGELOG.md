...
gets clean bands. Config: `force_ratio_mode` (`crop` default / `extend`), env
`CZ_FORCE_RATIO_MODE`. Validated on GPU: 512×512 → 910×512 (16:9, ratio 1.777), scene
continued plausibly; unit tests cover the geometry (both axes + no-op), the setter and
the UI radio mapping (`tests/test_force_ratio.py`).
