...
gets clean bands. A **harmonize pass** then blends the band seams: one light img2img pass
over the whole extended image (denoise `force_ratio_extend_denoise`, default **0.22**,
0 = off) removes the exposure/texture junction lines the raw outpaint leaves, while
keeping the original centre intact. Config: `force_ratio_mode` (`crop` default /
