...
meta tensor)`, and cost a full model reload every time. Found on crispz-klein (1.36.6),
...
diffusers also removes the offload hooks before loading a LoRA and puts them back after.
When the load fails in between, nobody puts them back: the pipeline stays on the CPU, its
execution device becomes `cpu`, and every render after that fails on `Cannot generate a
cpu tensor from a generator of type cuda`. A failed load now restores the offload in
place, and the cached base pipeline is checked before it is handed out
(`restore_offload`), so a pipeline left on the CPU repairs itself instead of asking for a
restart.
...
do not touch now stay as they are. `update.bat` now ends with an explicit exit
