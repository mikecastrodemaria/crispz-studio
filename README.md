...
- **🔌 Family CLI protocol v1** (`czp.bat` / `cz_protocol.py`): a machine-first
  ...
  an omni/edit model — `caps.supports.edit`), `inpaint` (image + mask +
  local prompt: only the white area is redrawn, every engine). A broken `config.txt` (invalid
...
- **Asset Browser** (standalone gallery, new tab): opens **instantly** (indexing +
  ...
  **🖼️ icon** next to each **LoRA** dropdown and the **Z-Image checkpoint** dropdown
  ...
  The badge only counts versions published for the **same base model**: a LoRA whose page
  gains a Krea2 or Flux release is not an update for your Z-Image copy.
